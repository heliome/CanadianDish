...
def about(site):
    ...
    if len(ingr) == len(bonus):
        s=0
        for i in bonus:
            k = ingr[s].text.replace(u'\xa0', u' ').replace("⅓", "1/3").split("\n")
            k.pop(-1)
            ingredients[i] = k
            s +=1
    else:
        k = ingr[0].text.replace(u'\xa0', u' ').replace("⅓", "1/3").split("\n")
        k.pop(-1)
    ...
    return pdf.create_pdf("img.jpg",name, prep[0], prep[1], prep[2], ingredients, steps, tips)
